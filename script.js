...
function request() {
  ...
  const url = 'https://fr.wikipedia.org/w/api.php?action=query&origin=*&pilicense=any&prop=pageimages&pithumbsize=250&format=json&gsrlimit=100&generator=search&gsrsearch=' + searchThis.value + '';
  ...
  for (var pageId in data.query.pages) {
    if (data.query.pages.hasOwnProperty(pageId)) {
      ...
      try {
        ...
        imagesArray.push('https://fr.wikipedia.org/?curid=' + pageId);
      }
      catch (err) {
